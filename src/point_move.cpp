...
int main(int argc, char** argv)
{
    ...
    // Create a ROS logger
    auto const logger = rclcpp::get_logger("point_move");
    ...
    std::map<std::string, double> joint_map = {
        {"ur10e_shoulder_pan_joint", 6.01},
        {"ur10e_shoulder_lift_joint", -2.09},
        {"ur10e_elbow_joint", -1.462},
        {"ur10e_wrist_1_joint", -0.59},
        {"ur10e_wrist_2_joint", 1.49},
        {"ur10e_wrist_3_joint", 3.205}
    };
    ...
    auto const initial_pose = []{
        ...
        msg.orientation.w = 0;
        msg.position.x = -0.942;
        msg.position.y = 0.435;
        msg.position.z = 0.480;
        return msg;
    }();
    ...
    for (int i = 0; i < 20; i++) {
        ...
        // Uncomment the following to move in different directions
        // pose.position.y += 0.01;
        // pose.position.z += 0.01;

        waypoints.push_back(pose);

        RCLCPP_INFO(logger, "Waypoint %d: x %.4f, y %.4f, z %.4f", i, pose.position.x, pose.position.y, pose.position.z);
    }
